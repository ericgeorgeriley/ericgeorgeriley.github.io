...
let player = {
    ...
    x: 20,
    y: 200,
    spr: document.getElementById("camper")
};
let colisionbox1 = {
    width: 30,
    height: 32,
...
let status = "stop";
let mg = 56;
function camper(x, y, width, height) {
    //ground
    ...
    ctx.drawImage(player.spr, 0, mg, 64, 64, x, y, 64, height);
    ctx.beginPath();
}
let i = 0;
function changSpr() {
...
function drawCamper() {
    camper(player.x, player.y, player.width, player.height);
}
let playerUp = false;
...
function generate(doubleGenRatio = 0.2, regen = time, repeat = true) //20%
{
    setTimeout(() => {
        ...
        let doubleGen = Math.random() < doubleGenRatio; 
        let v = Math.floor(Math.random() * 2) + 1;
        cacs.push(new cacy(cac["v" + v]));
           
        if(doubleGen) {
            generate(0,(time/10),false);
            console.log("DOUBLE GEN!", doubleGenRatio, regen);
        }
        
    if(repeat)
        generate(doubleGen ? (doubleGenRatio - 0.05) : doubleGenRatio);
